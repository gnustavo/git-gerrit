...
=item * implement 'git gerrit web [CHANGE]'

To go to CHANGE's Gerrit web page in a browser.
...
=item * Let the user set defaults for options in configuration variables

Something like this:

    git-gerrit.option.push.keep   1
    git-gerrit.option.push.rebase 0
    git-gerrit.option.ALL.debug   1
